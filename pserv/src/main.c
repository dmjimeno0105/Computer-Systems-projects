...
static void * connection_handler(void *args){
    ...
    //sets up client with socket
    http_setup_client(&client, bufio_create(*socket));
    bool online = true;
    while(online){
        online = http_handle_transaction(&client);
    }
...
server_loop(char *port_string)
{
    ...
    while (accepting_socket != -1) {
        ...
        //allocates space for a new socket
        int *soc = malloc(sizeof(int));
        *soc = client_socket;

        //creates a thread to handle a new client connection
        pthread_create(&client_connection,NULL,connection_handler,soc);
        // struct http_client client;
